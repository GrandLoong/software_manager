    ...
    def run(self):
        try:
            if sys.platform == "darwin":
                ...
                if ret_code != 0:
                    raise RuntimeError("Screen capture tool returned error code %s" % ret_code)

            elif sys.platform == "linux2":
                # use image magick
                ret_code = os.system("import %s" % self._path)
                if ret_code != 0:
                    raise RuntimeError("Screen capture tool returned error code %s. "
...
def screen_capture_file(output_path=None, autosize=True):
    """
    ...
    if autosize:
        background = QtGui.QImage(480, 480, QtGui.QImage.Format_ARGB32)
        background.fill(QtCore.Qt.black)
        painter = QtGui.QPainter(background)
        #painter.setCompositionMode(QPainter::CompositionMode_SourceOver);
        image = pixmap.toImage().scaled(background.size().width(),
                                        background.size().height(),
                                        QtCore.Qt.KeepAspectRatio)

        xstart = (background.size().width() - image.size().width()) / 2
        ystart = (background.size().height() - image.size().height()) / 2
